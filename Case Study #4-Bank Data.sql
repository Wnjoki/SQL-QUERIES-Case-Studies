...
VALUES
  ...
  /**A Customer Nodes Eploration**/
  /**1.How many unique nodes are there on the Data Bank system?**/
  SELECT COUNT(DISTINCT node_id) AS unique_codes
  FROM customer_nodes;

/**2.What is the number of nodes per region?**/
SELECT r.region_id,
       r.region_name,
       COUNT(n.node_id) AS number_of_nodes
FROM customer_nodes n
JOIN regions r ON n.region_id = r.region_id
GROUP BY r.region_id,n.region_id,r.region_name
...
SELECT customer_id,
       AVG(DATEDIFF(start_date,end_date)) avg_days
FROM customer_nodes
...
SELECT txn_type,
	   COUNT( txn_type) AS unique_counts,
...
SELECT AVG(historical_count) AS Avg_historical_count,
       AVG(deposit_amount) AS Avg_total_deposit
FROM (SELECT
	customer_id,
	txn_type,
	COUNT(txn_type) AS historical_count,
	SUM(txn_amount) AS deposit_amount
FROM customer_transactions 
WHERE txn_type = 'deposit'
GROUP BY customer_id, txn_type) deposit;
...
FROM
    (SELECT 
	   month(txt_date) AS month_id,
       DATENAME(M, txn_date) AS month_name,
       customer_id,
