...
ROUND(COUNT(*) * 100/(SELECT COUNT(DISTINCT customer_id) FROM subscriptions s),1)as churn_percent
FROM subscriptions s
JOIN plans p ON s.plan_id = p.plan_id
WHERE s.plan_id = 4;
...
FROM 
   (SELECT 
    Ntile(annual_plan.annual_date - trial_plan.trial_date, 0, 360, 12) AS  avg_days_to_upgrade
	 ...
	 ON trial_plan.customer_id = annual_plan.customer_id)bins
 ...
 SELECT 
  COUNT(*) AS downgraded,
    ...
    LEAD(plan_id, 1) OVER(
      PARTITION BY customer_id 
      ORDER BY plan_id) as next_plan
  FROM subscriptions) AS next_plan_cte
FROM next_plan_cte
WHERE start_date <= '2020-12-31'
  AND plan_id = 2 
  AND next_plan = 1;
